...
# --- CONFIGURATION ---
# Scenario: Base Coarsening (No Boost, No Feature Sim, No Fine-Tuning)
BOOST_H=0.0
USE_FEATURE_SIM=false
FT_EPOCH=0

N=3              # Number of runs per dataset
OUT_DIR="FinalLogFiles/ablation_base"
mkdir -p "$OUT_DIR"
...
echo "Starting Ablation Study [BASE] (N=$N runs per dataset)..."
...
for ds in "${DATASETS[@]}"; do
    ...
    # Convert ratio to percentage for filename (e.g., 0.75 -> 75)
    RATIO_PCT=$(echo "$RATIO * 100" | awk '{print int($1)}')

    for i in $(seq 1 $N); do
        TIMESTAMP=$(date +"%H%M%S")
        # Descriptive filename containing parameters
        FO_NAME=${FAN_OUT//,/_}
        ...
        echo "--> [Run $i/$N] Base Coarsening and Training $ds..."
        
        # Start Log File with Argument Summary
        {
            echo "===================================================="
            echo "ABLATION SCENARIO: BASE"
            echo "DATASET:           $ds"
            echo "RATIO:             $RATIO"
...
echo "Base ablation complete! Results are in '$OUT_DIR'."
