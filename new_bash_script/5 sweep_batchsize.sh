...
N=3
OUT_DIR_BASE="FinalLogFiles/sweep_batchsize"
...
for ds in "${DATASETS[@]}"; do
    for bs in "${BATCH_SIZES[@]}"; do
        ...
        for i in $(seq 1 $N); do
            ...
            if [ $? -eq 0 ]; then
                python node_classification.py --dataset $ds --mode $MODE --epoch 100 --ft_epoch $FT_EPOCH --num_layers 2 --fan_out $FAN_OUT --batch_size $bs $EXTRA_ARGS >> "$OUTPUT_FILE" 2>&1
            fi
        done
    done
done
