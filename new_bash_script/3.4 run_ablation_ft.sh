...
# --- CONFIGURATION ---
# Scenario: Fine-Tuning (FT enabled, No Boost, No Feature Sim)
BOOST_H=0.0
USE_FEATURE_SIM=false
FT_EPOCH=5

N=3              # Number of runs per dataset
OUT_DIR="FinalLogFiles/ablation_ft"
mkdir -p "$OUT_DIR"
...
echo "Starting Ablation Study [FT] (N=$N runs per dataset)..."
...
for ds in "${DATASETS[@]}"; do
    ...
    for i in $(seq 1 $N); do
        ...
        echo "--> [Run $i/$N] FT Coarsening and Training $ds..."
        
        # Start Log File with Argument Summary
        {
            echo "===================================================="
            echo "ABLATION SCENARIO: FT"
            echo "DATASET:           $ds"
            echo "RATIO:             $RATIO"
...
echo "FT ablation complete! Results are in '$OUT_DIR'."
