...
# --- CONFIGURATION ---
# Scenario: Boosting (Boost_H enabled, No Feature Sim, No Fine-Tuning)
BOOST_H=10000.0
USE_FEATURE_SIM=false
FT_EPOCH=0

N=3              # Number of runs per dataset
OUT_DIR="FinalLogFiles/ablation_boost"
mkdir -p "$OUT_DIR"
...
echo "Starting Ablation Study [BOOST] (N=$N runs per dataset)..."
...
for ds in "${DATASETS[@]}"; do
    ...
    for i in $(seq 1 $N); do
        ...
        echo "--> [Run $i/$N] Boost Coarsening and Training $ds..."
        
        # Start Log File with Argument Summary
        {
            echo "===================================================="
            echo "ABLATION SCENARIO: BOOST"
            echo "DATASET:           $ds"
            echo "RATIO:             $RATIO"
...
echo "Boost ablation complete! Results are in '$OUT_DIR'."
