...
def train(args, device, g_coarse, model, num_classes, is_multilabel, run_id, fine_tune_g=None):
    ...
    for epoch in range(args.epoch):
        ...
        if acc > best_val_acc:
            best_val_acc = acc; torch.save(model.state_dict(), ckpt_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lr", type=float, default=1e-3); parser.add_argument('--path', type=str, default='/data/dgl_lab')
    args = parser.parse_args(); args.dataset = args.dataset.strip().strip(',')
    ...
    result = {
        "dataset": args.dataset, "method": "ESSC_GCN", "run_id": run_id, "ratio": float(coarse_nodes / orig_nodes),
