...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="GCN training on original graph")
    ...
    if not torch.cuda.is_available():
        args.mode = "cpu"
    print(f"Training GCN in {args.mode} mode on original graph.")
    ...
    model = GCN(in_size, 256, num_classes, args.num_layers).to(device)

    if args.dt == "bfloat16":
        g = dgl.to_bfloat16(g)
        model = model.to(dtype=torch.bfloat16)

    print(f"\n--- Training GCN on Original Graph (on {args.mode}) ---")
    ...
    print(f"\n--- Testing GCN on Original Graph (on {args.mode}) ---")
