...
class GCN(nn.Module):
    ...
    def forward(self, blocks, x):
        ...
        for l, (layer, block) in enumerate(zip(self.layers, blocks)):
            h = layer(block, h)
            if l != len(self.layers) - 1: h = self.dropout(h)
        return h
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lr", type=float, default=1e-3); parser.add_argument('--path', type=str, default='/data/dgl_lab'); args = parser.parse_args()
    ...
    model = GCN(g.ndata["feat"].shape[1], args.hid_size, num_classes, args.num_layers).to(device)
    ...
    for epoch in range(args.epoch):
        ...
        if val_acc > best_val_acc: best_val_acc = val_acc; torch.save(model.state_dict(), f"best_baseline_gcn_{args.dataset}_{run_id}.pt")
        print(f"Epoch {epoch:03d} | Loss {total_loss/(it+1):.4f} | Val Acc: {val_acc:.4f}")
    train_time = time.time() - train_start
    ckpt_path = f"best_baseline_gcn_{args.dataset}_{run_id}.pt"
    ...
    result = {
        "dataset": args.dataset, "method": "BASELINE_GCN", "run_id": run_id, "ratio": 1.0,
