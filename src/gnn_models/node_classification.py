...
def train(args, device, g_coarse, model, num_classes, is_multilabel, run_id, fine_tune_g=None):
    ...
    fan_out_list = [int(f) for f in args.fan_out.split(",")]
    ...
    sampler = NeighborSampler(fan_out)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-r", "--retain_fraction", type=float, default=0.1)
    parser.add_argument("--boost_h", type=float, default=0.1)
    parser.add_argument("--use_feature_sim", action="store_true")
