...
if __name__ == "__main__":
    ...
    # For OGBN, Reddit, Pubmed, dataset.train_idx etc. are directly available or need extraction.
    # For Yelp, masks are set on g.ndata.
    # For IGB, masks are on g.ndata.
    # We will ensure train_idx, val_idx, test_idx are extracted from g.ndata masks for consistency
    # and passed to a dummy object if not already part of a 'dataset' object with these attributes.

    # Ensure dataset object has train_idx, val_idx, test_idx properties for train function
    if not hasattr(dataset, 'train_idx'):
        ...
        # Standardize train/val/test splits by getting them from the graph's boolean masks.
        ...
        # Handle cases where val_idx is empty (e.g., Yelp)
    ...
    out_size = num_classes # Use the local num_classes variable
    ...
    print(f"\n--- Training on Original Graph (on {args.mode}) ---")
