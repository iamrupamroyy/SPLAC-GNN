...
for ds in "${DATASETS[@]}"; do
    for fo in "${FAN_OUTS[@]}"; do
        ...
        for i in $(seq 1 $N); do
            ...
            if [ $? -eq 0 ]; then
                python node_classification.py --dataset $ds --mode $MODE --epoch $EPOCH --num_layers 2 --fan_out $fo --batch_size $BATCH_SIZE -r $RATIO --boost_h $BOOST_H --use_feature_sim $EXTRA_ARGS >> "$OUTPUT_FILE" 2>&1
            fi
        done
    done
done
